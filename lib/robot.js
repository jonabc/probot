...
class Robot {
  ...
  /**
   * Retrieve the installation token for an installation id
   *
   * **Note**: `robot.installationToken` is asynchronous, so it needs to be prefixed with a
   ...
   *      const installationToken = await robot.installationToken(context.payload.installation.id);
   *    });
   *  };
   *
   ...
   *
   * @returns {Promise<string>} - An installation token that can be used for authenticating requests to GitHub.
   */
